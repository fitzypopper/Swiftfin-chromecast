...
typedef NS_ENUM(NSInteger, GCKErrorCode) {
   ...
   * another action has preempted it. Don't attempt to re-try a connection that failed with
   * GCKErrorCodeCancelled. Doing so may lead to unexpected behavior.
   */
  ...
  /**
   * Error code indicating that the SDK failed to wake a Wake-on-LAN device.
   */
  GCKErrorCodeUnableToWakeSleepingDevice = 109,

  /**
   * Error code indicating that the SDK failed to wake a Wake-on-LAN device, and has exceeded the number of failed wake attempts.
   */
  GCKErrorCodeExceededMaxFailedWOLConnectionAttempts = 110,
};
